class InputDesign extends HTMLElement {
	...
	setupEventListeners() {
		...
		this.clearButton.addEventListener('mouseenter', () => {
			if (this.inputElement.value.trim() !== '') {
				// Если input не пустой, то добавляем класс(hover) input-у и контейнеру для изменения background
	...
	updateClearButtonState(value = '') {
		const flagDisabled = value.trim() === '';
		this.clearButton.disabled = flagDisabled;
		this.clearButton.style.cursor = flagDisabled ? 'auto' : 'pointer';
